...
bool AngelScriptEditor::_wasScintillaModuleLoaded = false;
bool AngelScriptEditor::_wasLexillaModuleLoaded = false;
Lexilla::CreateLexerFn AngelScriptEditor::_createLexer = nullptr;

INT_PTR CALLBACK AngelScriptEditorDlgProc(
	HWND hwndDlg,
	UINT message,
	WPARAM wParam,
	LPARAM lParam
...
{
	AngelScriptEditor* editor = nullptr;

	if (message == WM_INITDIALOG)
	{
		editor = reinterpret_cast<AngelScriptEditor*>(lParam);
	...
	editor = reinterpret_cast<AngelScriptEditor*>(GetWindowLongPtr(hwndDlg, GWLP_USERDATA));
	...
	case WM_COMMAND:
		...
		case IDC_BUTTON1:
		{
			...
			try
			{
				...
				std::string err;
				if (!editor->_script->TryCompile(err))
				{
					Logging::PrintWarning(err);
...
void* CreateAngelScriptEditorWindow(HWND dlgOwner, HINSTANCE hInstance, void* reflectionData, void* contentManager, void* object)
{
	...
	auto obj = static_cast<AngelScriptObject*>(object);

	HMODULE moduleHandle = hInstance;
	GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
		reinterpret_cast<LPCSTR>(&CreateAngelScriptEditorWindow),
		reinterpret_cast<HMODULE*>(&moduleHandle)
	);

	AngelScriptEditor editor(dlgOwner, moduleHandle, obj);
	return static_cast<void*>(editor.ShowDialog());
}

AngelScriptEditor::AngelScriptEditor(
	HWND dlgOwner,
	HINSTANCE hInstance,
...
void AngelScriptEditor::_createNewObject(AngelScriptObject* object)
{
	if (object == nullptr)
	{
		_script = new AngelScriptObject();
		_wasAllocated = true;
	}
}
...
void AngelScriptEditor::initForms()
{
	...
	SendMessageA(_codeControl, SCI_SETKEYWORDS, 0, reinterpret_cast<WPARAM>(
		"and auto break case cast catch class const continue default do "
		"else enum false for foreach funcdef if import in inout int interface "
		"is mixin namespace null private protected return "
		"switch true try typedef using while"
		"abstract delete explicit external final from function get override "
		"property set shared super this"
		));

	SendMessageA(_codeControl, SCI_SETKEYWORDS, 1, reinterpret_cast<WPARAM>(
		"int8 int16 int32 int64 uint uint16 uint32 uint64 void double bool float"
		));
	...
	{
		_loadCodeFromFile("Scripts\\Empty.as");
	}
}

void AngelScriptEditor::handleCharAdded(HWND scintilla, int ch)
{
	...
	if (lenEntered > 0)
	{
		...
		if (!autoCActive)
		{
			...
			const char* keywordList =
				...
				"else enum false float for foreach funcdef if import in inout int interface "
				"int8 int16 int32 int64 is mixin namespace not null or private protected return "
				"switch true try typedef uint uint16 uint32 uint64 using void while xor"
				"abstract delete explicit external final from function get override "
				"property set shared super this";
...
void AngelScriptEditor::handleTextChanged(HWND scintilla)
{
...
AngelScriptObject* AngelScriptEditor::ShowDialog()
{
	INT_PTR dlg = DialogBoxParam(
		_hInstance,
		MAKEINTRESOURCE(ID_ANGELSCRIPTDIALOG),
		_owner,
		AngelScriptEditorDlgProc,
...
std::filesystem::path AngelScriptEditor::openFileDialog()
{
	OPENFILENAMEA ofn{};
	char fileName[MAX_PATH] = { 0 };
	ofn.lStructSize = sizeof(ofn);
...
void AngelScriptEditor::_loadCodeFromFile(const std::filesystem::path& path)
{
...
void AngelScriptEditor::openFile()
{
...
void AngelScriptEditor::save(const std::filesystem::path& path)
{
