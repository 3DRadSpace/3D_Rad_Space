	...
	gdevice(device)
{
	...
	if(FAILED(r)) throw ResourceLoadingError(Tag<Texture2D>{}, filename, "");
#endif
}
...
void Texture2D::SaveToFile(const std::string &path)
{
	...
	MultiByteToWideChar(CP_ACP, 0, path.c_str(), int(path.length()), wpath, -1);

	HRESULT r = DirectX::SaveWICTextureToFile(gdevice->context.Get(), texture.Get(), GUID_ContainerFormatPng, wpath, nullptr, nullptr, true);
