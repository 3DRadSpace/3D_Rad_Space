...
namespace Engine3DRadSpace::Reflection
{
	...
	template<ReflectableType T, size_t N>
	class E3DRSP_REFLECTION_EXPORT ReflectedField<std::array<T, N>> final : public IReflectedField
	{
	public:
	...
	public:
		ReflectedField(
			...
			const std::string& visibleName,
			const std::string& description,
			const Array &defaultValues
		) :
			IReflectedField(offset_obj_field, sizeof(Array), visibleName, description, typeid(Array)),
			_defaultVal(defaultValues)
		{
		}
		...
		[[nodiscard]] const void* Get(void* objPtr) const override
		{
			...
			return std::launder(reinterpret_cast<Array*>(static_cast<std::byte*>(objPtr) + _offset));
		}
		/// <summary>
		/// Sets the field value.
		/// </summary>
		...
		void Set(void* objPtr, const void* value) const override
		{
			...
			Array* lhs = std::launder(reinterpret_cast<Array*>(static_cast<std::byte*>(objPtr) + _offset));
			const Array* rhs = static_cast<const Array*>(value);
		...
		std::unique_ptr<IReflectedField> Clone() const override
		{
			return std::make_unique<ReflectedField<std::array<T, N>>>(_offset, _name, _desc, _defaultVal);
		}
