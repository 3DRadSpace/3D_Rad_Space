...
EditorWindow::EditorWindow(HINSTANCE hInstance, PWSTR cmdArgs)
{
	...
	*/
	...
	//create windows
	MainWindow = CreateWindowEx(0, MainWindowClassName, TEXT("3DRadSpace - Editor"), WS_OVERLAPPEDWINDOW | WS_VISIBLE, 0, 0, resolution.X, resolution.Y, nullptr, MainMenu, hInstance, 0);
	RenderWindow = CreateWindowEx(WS_EX_ACCEPTFILES, EditorWindowClassName, TEXT(""), WS_CHILD, 125, 25, 800, 600, MainWindow, nullptr, hInstance, 0);
	...
	*/
	...
	HIMAGELIST toolBarImageList = ImageList_Create(16, 16, ILC_COLOR32 | ILC_MASK, 7, 0);
...
void EditorWindow::RenderUpdateLoop()
{
	...
	while (!ShouldExit)
	{
		...
		if (mouse.leftButton && (focusWindow == MainWindow || focusWindow == RenderWindow))
		{
			...
			pMouseDelta = center - Point(mousePos.x, mousePos.y);

			SetCursorPos(center.X, center.Y);

			camRotX -= pMouseDelta.X * 5 * update_dt.count();
			camRotY -= pMouseDelta.Y * 5 * update_dt.count();
...
Point EditorWindow::GetCenter()
{
	RECT r{};
	GetWindowRect(RenderWindow, &r);
	int width = r.right - r.left;
	int height = r.bottom - r.top;
	return Point(r.left + (width / 2), r.top + (height / 2));
}
