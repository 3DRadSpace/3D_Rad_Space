...
namespace Lib3DRadSpace_DX
{
    ...
    /// </summary>
    public class FirstPersonCamera : BaseGameObject, IRenderer,IPhysicsObject
    {
        /// <summary>
        ...
        public new Vector3 Position
        {
            get
            {
                return BEPU2XNA.XNAVector(_controller.Body.Position);
            }
            set
            {
                _controller.Body.Position = BEPU2XNA.BEPUVector(value);
            }
        }
        ...
        public Matrix Projection
        {
            get
            {
                return Matrix.CreatePerspectiveFieldOfView(FOV, CurrentProject.Resolution.X / CurrentProject.Resolution.Y, _npd, _fpd);
            }
        }
        /// <summary>
        ...
        {
            return ray.Position + (ray.Direction*ray.Intersects(new BoundingSphere(Position, Height)));
        }
        ...
        public void ApplyForce(Vector3 dir, ForceType t, float intensity)
        {
            switch(t)
            {
                case ForceType.Acceleration:
                {
                    BEPUutilities.Vector3 v = BEPU2XNA.BEPUVector(dir*intensity);
                    _controller.Body.ApplyLinearImpulse(ref v);
                    return;
                }
                case ForceType.Spin:
                {
                    BEPUutilities.Vector3 v = BEPU2XNA.BEPUVector(dir * intensity);
                    _controller.Body.ApplyAngularImpulse(ref v);
                    return;
                }
                default: return;
            }    
