...
namespace Lib3DRadSpace_DX
{
	...
	/// </summary>
	public class Camera : BaseGameObject,IRenderer
	{
		...
		public Vector3 Target
		{
			...
			set
			{
				...
				else throw new InvalidOperationException("Cannot set the camera target location when the camera is using a quaternion or euler angles to describe it's own rotation.");
			}
		}
		Vector3 _target;
		/// <summary>
		...
		public Matrix View
		{
			get
			{
				if (!LookAtLocation) return Matrix.CreateLookAt(_pos, Target, Up);
				else return Matrix.CreateLookAt(_pos, _target, Up);
			}
		}
		/// <summary>
		...
		public new Matrix World
		{
			get
			{
				return Matrix.CreateFromYawPitchRoll(RotationEuler.Y, RotationEuler.X, RotationEuler.Z) * Matrix.CreateTranslation(_pos);
			}
		}
		/// <summary>
		...
		public override void SaveF(List<byte> buff)
		{
			...
			ByteCodeParser.SetVector3(buff, _pos);
			ByteCodeParser.SetVector3(buff, RotationEuler);
			ByteCodeParser.SetVector3(buff, Up);
			ByteCodeParser.SetVector3(buff, new Vector3(FOV, _npd, _fpd));
		}
	}
}
