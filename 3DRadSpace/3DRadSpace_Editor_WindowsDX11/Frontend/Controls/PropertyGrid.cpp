...
// Link with required libraries for Win32 controls and shell API
...
{
    if(NULL == szzStr || _T('\0') == *szzStr) szzStr = TEXT("");
...
static VOID ListBox_OnKillFocus(HWND hwnd, HWND hwndNewFocus)
{
    if (NULL != g_lpInst->lpCurrent)
    {
        if(PIT_CHECK == g_lpInst->lpCurrent->iItemType)//DWM 1.3: Added
            AllocatedString_Replace(g_lpInst->lpCurrent->lpszMisc, TEXT("F"));
    }
...
static HWND CreateToolTip(HINSTANCE hInstance, HWND hwndParent)
{
    ...
    ti.hinst = hInstance;
    ti.uId = 0;
    static TCHAR emptyTip[] = TEXT("");
    ti.lpszText = emptyTip;
...
static HWND CreateEdit(HINSTANCE hInstance, HWND hwndParent, INT id)
{
    ...
    hwnd = CreateWindowEx(dwExStyle, WC_EDIT, TEXT(""), dwStyle, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, hwndParent, (HMENU)(UINT_PTR)id, hInstance, NULL);
...
static HWND CreateIpEdit(HINSTANCE hInstance, HWND hwndParent, INT id, LPRECT lprc)
{
    ...
    hwnd = CreateWindowEx(dwExStyle,
        ...
        hwndParent, (HMENU)(UINT_PTR)id, hInstance, NULL);
...
static HWND CreateDatePicker(HINSTANCE hInstance, HWND hwndParent, INT id, BOOL fDate)
{
    ...
    hwnd = CreateWindowEx(dwExStyle, DATETIMEPICK_CLASS, TEXT(""), dwStyle, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, hwndParent, (HMENU)(UINT_PTR)id, hInstance, NULL);
...
static VOID FileDialogItem_FromString(LPPROPGRIDFDITEM lpPgFdItem, LPTSTR lpszFdItem)
{
    ...
    // Use static buffers to hold the values
    static TCHAR s_DlgTitle[MAX_PATH] = {0};
    static TCHAR s_FilePath[MAX_PATH] = {0};
    static TCHAR s_Filter[MAX_PATH] = {0};
    static TCHAR s_DefExt[8] = {0};
    _tcsncpy(s_DlgTitle, PgFdItem[0], MAX_PATH-1);
    _tcsncpy(s_FilePath, PgFdItem[1], MAX_PATH-1);
    _tcsncpy(s_Filter, PgFdItem[2], MAX_PATH-1);
    _tcsncpy(s_DefExt, PgFdItem[3], 7);
    lpPgFdItem->lpszDlgTitle = s_DlgTitle;
    lpPgFdItem->lpszFilePath = s_FilePath;
    lpPgFdItem->lpszFilter = s_Filter;
    lpPgFdItem->lpszDefExt = s_DefExt;
}
...
static LPTSTR BrowseFolder(HWND hwnd, LPTSTR curPath, LPTSTR title, LPTSTR rootPath)
{
    ...
    if (SUCCEEDED(SHGetMalloc(&pMalloc)))
    {
        ...
        bi.hwndOwner = hwnd;
        bi.pszDisplayName = 0;
        // Avoid assigning string literal directly to LPTSTR
        static TCHAR s_title[MAX_PATH] = {0};
        if (title && *title)
            _tcsncpy(s_title, title, MAX_PATH-1);
        else
            s_title[0] = 0;
        bi.lpszTitle = s_title;
        bi.pidlRoot = ConvertPathToLpItemIdList(rootPath);
        bi.ulFlags = BIF_RETURNONLYFSDIRS | BIF_STATUSTEXT;
        bi.lpfn = BrowseCallbackProc;
        bi.lParam = (LPARAM)curPath;
    ...
    else
    {
        ...
        if (LB_ERR != idx)
        {
            ...
            for (INT i = ListBox_GetCount(g_lpInst->hwndListMap) - 1; 0 <= i; i--)
            {
                ...
                if (NULL != pp && NULL != pp->lpszCatalog)
                {
...
static VOID ListBox_OnEndScroll(HWND hwnd)
{
    ...
    switch (g_lpInst->lpCurrent->iItemType)
    {
        ...
        case PIT_CHECK:
            AllocatedString_Replace(g_lpInst->lpCurrent->lpszMisc, TEXT("T"));
        ...
        if (NULL != g_lpInst->lpCurrent)
        {
            ...
            switch (g_lpInst->lpCurrent->iItemType)
            {
                ...
                case PIT_STATIC:
                    break; //Ignore
                case PIT_CHECK:
                    AllocatedString_Replace(g_lpInst->lpCurrent->lpszMisc, UNSELECT); //Prevent toggle
                    break;
                case PIT_DATETIME:
...
static VOID ListBox_OnSelectIP(HWND hwnd, RECT rc, LPLISTBOXITEM pItem)
{
    ...
    _stscanf(pItem->lpszCurValue, _T("%u.%u.%u.%u"), &ip0, &ip1, &ip2, &ip3);
...
static VOID ListBox_OnKeyDown(HWND hwnd, UINT vk, BOOL fDown, INT cRepeat, UINT flags)
{
    ...
    if (NULL != g_lpInst->lpCurrent)
    {
        ...
        if (PIT_CHECK == g_lpInst->lpCurrent->iItemType && (VK_SPACE == vk || VK_RETURN == vk))
        {
            if (0 == _tcsicmp(g_lpInst->lpCurrent->lpszMisc, SELECT))
            {
                if (0 == _tcsicmp(g_lpInst->lpCurrent->lpszCurValue, CHECKED))
                    AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, TEXT("F"));
                else
                    AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, TEXT("T"));
    ...
    if (ID_BUTTON == id)
    {
        ...
        switch (g_lpInst->lpCurrent->iItemType)
        {
            ...
            case PIT_FILE:
            {
                ...
                PROPGRIDFDITEM pgi;
                pgi.lpszDlgTitle = (LPTSTR)ofn.lpstrTitle;
                pgi.lpszFilePath = NewString(TEXT(""));
                pgi.lpszFilter = (LPTSTR)ofn.lpstrFilter;
                pgi.lpszDefExt = (LPTSTR)ofn.lpstrDefExt;
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, pgi.lpszFilePath);
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszMisc, FileDialogItem_ToString(&pgi));
            ...
            case PIT_FOLDER:
            {
                static TCHAR emptyStr[1] = { 0 };
                LPTSTR temp = BrowseFolder(hwnd, g_lpInst->lpCurrent->lpszCurValue, emptyStr, emptyStr);
...
static VOID Grid_OnSize(HWND hwnd, UINT state, INT cx, INT cy)
{
    ...
    if (NULL != g_lpInst->lpCurrent)
    {
        if(PIT_CHECK == g_lpInst->lpCurrent->iItemType)
            AllocatedString_Replace(g_lpInst->lpCurrent->lpszMisc, TEXT("F")); //Prevent toggle
    }
...
static BOOL Grid_OnSetCursor(HWND hwnd, HWND hwndCursor, UINT codeHitTest, UINT msg)
{
    if (NULL != g_lpInst->lpCurrent)
    {
        ...
        {
            if(PIT_CHECK == g_lpInst->lpCurrent->iItemType)
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszMisc, SELECT);
            ...
            case LBN_SELCHANGE:
            {
                ...
                if (iCurSel != g_lpInst->iPrevSel &&
                    0 == _tcsicmp(pItem->lpszMisc, SELECT))
                        AllocatedString_Replace(pItem->lpszMisc, UNSELECT);
                    }
                }
...
static INT Grid_OnAddString(LPPROPGRIDITEM pgi)
{
    ...
    {
        lpi = NewItem(pgi->lpszCatalog, NewString(_T("")), NewString(_T("")),
            NewString(_T("")), NewString(_T("")), PIT_CATALOG);
        lpi->fCollapsed = TRUE;
    ...
    switch (pgi->iItemType)
    {
        ...
        case PIT_FONT:
            ...
            break;
        case PIT_CHECK:
            lpszCurValue = (BOOL)pgi->lpCurValue ? NewString(CHECKED) : NewString(UNCHECKED);
            break;
        case PIT_FILE:
        ...
        case PIT_DATETIME:
            ...
            _tcscat(buf, _T(" "));
            GetTimeFormat(LOCALE_USER_DEFAULT, 0, (LPSYSTEMTIME)pgi->lpCurValue,
                _T("hh':'mm':'ss tt"), buf + _tcslen(buf), MAX_PATH - (int)_tcslen(buf));
            lpszCurValue = buf;
            break;
    }
    ...
    if (NULL != pItem)
    {
        if (PIT_CATALOG != pItem->iItemType)
        {
            ...
            if (LB_ERR != i)
            {
                ...
                if(PIT_CHECK == pItem->iItemType)
                    AllocatedString_Replace(pItem->lpszMisc, SELECT); //DWM 1.2: Ensure Checkbox selected
                    AllocatedString_Replace(pItem->lpszMisc, SELECT); //DWM 1.2: Ensure Checkbox selected
...
static LRESULT Grid_OnSetItemData(INT iItem, LPPROPGRIDITEM pgi)
{
    ...
    if (NULL != lpiCurrent)
    {
        ...
        lpiNew = NewItem(lpiCurrent->lpszCatalog, pgi->lpszPropName,
            (LPTSTR)lpszCurValue, pgi->lpszzCmbItems, pgi->lpszPropDesc, pgi->iItemType, pgi->lpUserData);
...
HWND New_PropertyGrid(HWND hParent, DWORD dwID)
{
    ...
    hPropertyGrid = CreateWindowEx(0, g_szClassName, _T(""),
         WS_CHILD | WS_TABSTOP | WS_VISIBLE, 0, 0, 0, 0, hParent, (HMENU)(UINT_PTR)dwID, hinst, NULL);
